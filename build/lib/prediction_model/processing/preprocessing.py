...
class MeanImputer(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        X = X.copy()
        for col in self.variables:
            X[col].fillna(self.mean_dict[col], inplace=True) #fill the missing values with mean values
...
class ModeImputer(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y=None):
        self.mode_dict = {}
        for col in self.variables: #for each column
            self.mode_dict[col] = X[col].mode()[0] #cal the mode for each feature
        return self
    
    def transform(self, X):
        X = X.copy()
        for col in self.variables:
            X[col].fillna(self.mode_dict[col], inplace=True) #fill the missing values with mode values
        return X
